...
if test -f /home/nexisato/miniconda3/bin/conda
    eval /home/nexisato/miniconda3/bin/conda "shell.fish" "hook" $argv | source
else
    if test -f "/home/nexisato/miniconda3/etc/fish/conf.d/conda.fish"
        . "/home/nexisato/miniconda3/etc/fish/conf.d/conda.fish"
    else
        set -x PATH "/home/nexisato/miniconda3/bin" $PATH
    end
end
# <<< conda initialize <<<
...
set -l PATH_COLON (string join ':' '' $PATH '')

if string match -q "*:$HOME/.cargo/bin:*" $PATH_COLON
    # do nothing
else
...
# Golang 环境变量
set -x GOPATH "$HOME/DevEnv/gopath"
set -x GOROOT "/usr/local/go"
set -x PATH $GOROOT/bin $PATH
set -x PATH $GOPATH/bin $PATH


## openssl 环境变量 (C++ & bin)- 这里是自行编译的openssl
set -x C_INCLUDE_PATH /usr/local/openssl/include $C_INCLUDE_PATH
set -x CPLUS_INCLUDE_PATH /usr/local/openssl/include $CPLUS_INCLUDE_PATH
set -x OPENSSL_ROOT_DIR /usr/local/openssl
set -x LIBRARY_PATH  /usr/local/openssl/lib64 $LIBRARY_PATH
set -x LD_LIBRARY_PATH /usr/local/openssl/lib64 $LD_LIBRARY_PATH
set -x PATH $OPENSSL_ROOT_DIR/bin $PATH
...
set -x MANPATH "/usr/local/texlive/2023"
set -x INFOPATH "/usr/local/texlive/2023/texmf-dist/doc/info" $INFOPATH

# Mojo
set -x MODULAR_HOME "/home/nexisato/.modular"
